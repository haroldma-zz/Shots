                        ...
                        using (var rnd = stream.AsRandomAccessStream())
                        {
                            ...
                            using (var source = new RandomAccessStreamImageSource(rnd, ImageFormat.Jpeg))
                            {
                                ...
                                using (var filters = new FilterEffect(source))
                                {
                                    ...
                                    var dispatcherUtility = App.Current.Kernel.Resolve<IDispatcherUtility>();

                                    // Create a target where the filtered image will be rendered to
                                    // Need to switch to UI thread for this
                                    var target = await dispatcherUtility.RunAsync(
                                        () => new WriteableBitmap((int) (bounds.Width),
                                            (int) (bounds.Height))).DontMarshall();
                                    ...
                                    using (var renderer = new WriteableBitmapRenderer(filters, target))
                                    {
                                        ...
                                        // Need to switch to UI thread for this
                                        await dispatcherUtility.RunAsync(() =>
                                        {
